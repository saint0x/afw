...
// OpenAI provider is defined inline below for now
// TODO: Move to separate providers module in Phase 2
...
impl LLMProvider for OpenAIProvider {
    ...
    async fn complete(&self, request: LLMRequest) -> AriaResult<LLMResponse> {
        ...
        Ok(LLMResponse {
            ...
            model: request.config.model.clone().unwrap_or_else(|| "gpt-4o".to_string()),
    ...
    async fn complete_stream(&self, _request: LLMRequest) -> AriaResult<Box<dyn futures::Stream<Item = AriaResult<LLMResponse>> + Unpin + Send>> {
        Err(AriaError::new(
            ErrorCode::LLMApiError,
            ErrorCategory::LLM,
...
impl LLMProvider for AnthropicProvider {
    ...
    fn supports_functions(&self) -> bool {
        false // Anthropic has different function calling support
    }
    ...
    async fn complete(&self, request: LLMRequest) -> AriaResult<LLMResponse> {
        ...
        Ok(LLMResponse {
            ...
            model: request.config.model.clone().unwrap_or_else(|| "claude-3-opus-20240229".to_string()),
