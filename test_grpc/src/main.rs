...
async fn main() -> Result<()> {
    println!("🚀 Testing Aria `new -> build -> upload` Workflow");
    println!("================================================\n");
    ...
    let new_output = Command::new("cargo")
        .args(&[
            "run", "--manifest-path", "../crates/ar-c/Cargo.toml", "--quiet", "--",
            "new", project_name,
        ])
    ...
    // --- Step 3: `arc upload` ---
    println!("3️⃣  Running `arc upload` and streaming output...");
    let mut upload_cmd = Command::new("cargo")
        .args(&[
            "run", "--manifest-path", "../../crates/ar-c/Cargo.toml", "--quiet", "--",
            "upload", "--bundle", bundle_file.to_str().unwrap(),
        ])
        .current_dir(&project_path)
        .stdout(Stdio::piped())
        .spawn()?;
